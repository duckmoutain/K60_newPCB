                                ...
                                uint16_t rcv_buf_size)
{
  ...
  if (idx == PROTOCOL_INTERFACE_MAX)
  {
    ...
    //PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	  //printf("PROTOCOL_ERR_OBJECT_NOT_FOUND6\n");
    return status;
  }
...
//    PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	 // printf("PROTOCOL_ERR_NOT_ENOUGH_MEM7\n");
    return status;
  }
...
//  PROTOCOL_OTHER_INFO_PRINTF("Interface %s[%d] has been initialized.",
  ...
  //printf("Interface has been initialized8\n");
...
//    PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	//  printf("PROTOCOL_ERR_INTER_NOT_FOUND8\n");
    return status;
  }
  ...
  if (perph->type == CAN_PORT)
  {
    ...
    else
    {
      status = PROTOCOL_ERR_INTER_NOT_FOUND;
//      PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	//	printf("PROTOCOL_ERR_INTER_NOT_FOUND10\n");
    }
  }
  else if (perph->type == COM_PORT)
  {
    ...
    else
    {
      status = PROTOCOL_ERR_INTER_NOT_FOUND;
//      PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	//	printf("PROTOCOL_ERR_INTER_NOT_FOUND11\n");
    }
  }
  return status;
}
...
{
  uint32_t status = PROTOCOL_SUCCESS;
  //protocol_rcv_data(p_data, data_len, &protocol_local_info.interface[0]);
...
int32_t protocol_set_route(uint8_t tar_add, const char *name)
{
  ...
  if (perph == NULL)
  {
    status = PROTOCOL_ERR_INTER_NOT_FOUND;
//    PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	 // printf("PROTOCOL_ERR_INTER_NOT_FOUND12\n");
    return status;
  }
...
//    PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	//  printf("PROTOCOL_ERR_ROUTEU_SET_BEYOND13\n");
    return status;
  }
  ...
  if (perph->is_valid == 0)
  {
    status = PROTOCOL_ERR_INTER_NOT_FOUND;
//    PROTOCOL_ERR_INFO_PRINTF(status, __FILE__, __LINE__);
	//  printf("PROTOCOL_ERR_INTER_NOT_FOUND14\n");
    return status;
  }
...
//  PROTOCOL_OTHER_INFO_PRINTF("Route has been set, Address 0x%02X next jump is %s[%d].",
  ...
  //printf("Route has been set, Address  next jump15\n");
